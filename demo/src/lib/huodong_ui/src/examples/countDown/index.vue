    ...
    <Example :info="exampleInfo.info"
      ...
      >
        <template slot-scope='scope'>
          <div class="counter">
              {{scope.current.date}}：{{scope.current.hour}}：{{scope.current.min}}：{{scope.current.sec}} : {{scope.current.ms}}
        </div>
        </template>
</countDown>
...
export default {
  data() {
    return {
      torrowTime: new Date().getTime()+10000,
      exampleInfo: {
        info: {
          title: "倒计时",
          author: "fupengcheng"
        ...
        code: `
                    <countDown 
                    ref='countDown'
                      :timeStamp='1515760000000'
                      :currentTimeStamp='1517750000000'
                      :interval = '10'
                      @timeUp='handleTimeUp'
                      @tick = 'handleTick'
                    >
        ...
        tableInfo: {
          attributes: [
            ...
            {
              propName: `currentTimeStamp`,
              explain: `倒计时开始时间，到毫秒位`,
              type: `string || number`,
